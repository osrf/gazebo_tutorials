# Install Gazebo from source (Ubuntu)
...
**Release Note:** in order to use DART, a full compilation of Gazebo from source is needed (as detailed in this document). The .deb packages are only shipping the ODE, Bullet, and Simbody physics engines.
   ...
   ***DART Support***
   ...
   Support for [DART](http://dartsim.github.io/) version 4.3 is integrated into Gazebo 5. In an Ubuntu system, several Personal Package Archives (PPA's) can be used to install the proper package and dependencies. Note that adding these PPA's may cause conflicts with ROS.
...
1. Change directory into the sdformat repository and switch to the 2.3 branch:

        cd ~/sdformat
        hg up sdf_2.3

     **Note:** the `default` branch is the development branch where you'll find the bleeding edge code, your cloned repository should be on this branch by default but we recommend you switch to the 2.3 branch if you desire more stability
...
1. Change directory in the Gazebo repository and switch to the 5 branch

        cd ~/gazebo
        hg up gazebo5

     **Note:** the `default` branch is the development branch where you'll find the bleeding edge code, your cloned repository should be on this branch by default but we recommend you switch to the 4.0 branch if you desire more stability
...
**Trusty and greater**
    ...
    echo "export LD_LIBRARY_PATH=<install_path>/local/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH" >> ~/.bashrc
    echo "export PATH=<install_path>/local/bin:$PATH" >> ~/.bashrc
    echo "export PKG_CONFIG_PATH=<install_path>/local/lib/x86_64-linux-gnu/pkgconfig:$PKG_CONFIG_PATH" >> ~/.bashrc
    source ~/.bashrc
...
If Gazebo was installed to `/usr/local/` and running gazebo throws an error similar to:
  ...
  , then `/usr/local/lib/x86_64-linux-gnu` is not in load path (default behavior for Ubuntu). Run the following commands and then try running gazebo again:
