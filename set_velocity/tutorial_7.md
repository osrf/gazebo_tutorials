...
# Examples
[Downloaded an example plugin here](http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/).
...
<include from="/          this->jointController\.reset/" to="/          this->jointController->SetVelocityTarget\(name, 1.0\);/" src="http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/src/SetJointVelocityPlugin.cpp"/>
...
<include from="/          \/\/ Link velocity instantaneously without applying forces/" to='/          model->GetLink\(\"white_link_2\"\)->SetAngularVel\({1, 0, 0}\);/' src="http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/src/SetLinkVelocityPlugin.cpp"/>
...
<include from="/        \/\/ create prismatic joint with the world as a parent/" to='/        this->joint->SetParam\(\"vel\", 0, magnitude\);/' src="http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/ode_perfect_linear.hh"/>
...
<include from="/        \/\/ create revolute joint with the world as a parent/" to='/        this->joint->SetParam\(\"vel\", 0, magnitude\);/' src="http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/ode_perfect_angular.hh"/>
...
<include from="/        \/\/ Create a phantom link/" to='/        this->revoluteJoint->SetParam\(\"vel\", 0, angularMagnitude\);/' src="http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/ode_perfect_velocity.hh"/>
...
<include from="/      \/\/ Hard coded gains\. Tune these for your own application!/" to='/      double angular_imax = 123456789\.0;/' src='http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/pid_link.hh' />
...
<include from="/      \/\/ Add a PID controller for each DoF/" to='/      }/' src='http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/pid_link.hh' />

Every physics update the error between the actual velocity and the target velocity needs to be given to the controller.

<include from="/      \/\/ Calculate the error between actual and target velocity/" to='/      worldTorque\.z = this->controllers\[5\]\.Update\(angularError\.z, dt\);/' src='http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/pid_link.hh' />

The controllers will output forces and torques that should be applied to the link to correct for the current error.

<include from="/      \/\/ Add those forces to the body/" to='/      this->link->AddTorque\(worldTorque\);/' src='http://github.com/osrf/gazebo_tutorials/raw/master/set_velocity/examples/set_vel_plugin/include/pid_link.hh' />
